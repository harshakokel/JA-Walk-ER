...
class MF(MainFrame):
    ...
    def onOpenGraph(self, event):
        ...
        except ExtParser.ParseException:
    ...
    def onExportModes( self, event ):
        ...
        #     self.is_data_changed = True
        # dlg.Destroy()
        fd = wx.FileDialog(self, "Save modes File As", "", "",
                           "modes Files (*.*)|*.*", wx.SAVE | wx.FD_SAVE)
        if fd.ShowModal() == wx.ID_OK:
            fp = fd.GetPath()
        else:
            return  ### User canceled.
        ...
        modes_fd = file(fp, "w+b")
        modes_fd.write(self.data_graph.EG_to_modes().decode('utf8'))
        modes_fd.close()
